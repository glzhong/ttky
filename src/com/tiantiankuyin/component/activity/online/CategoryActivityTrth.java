...
public class CategoryActivityTrth extends Activity {
	public static final String ACTIVITY_ID = "CategoryActivityTrth";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		CommonUtils.setTitle(rootView, "30元专属", true, false);
		...
		btn_download.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				for (int i = 0; i < listStr.size(); i++) {
				 ...
				 //download("");
				if(listStr.size()>20){
					Toast.makeText(CategoryActivityTrth.this, "最多可以选中20首歌曲", Toast.LENGTH_LONG).show();
				}else if(listStr.size()==0){
					Toast.makeText(CategoryActivityTrth.this, "请选中你喜欢的歌曲", Toast.LENGTH_LONG).show();
				}else{
					if(mSharedPreferencesManager.readIntPreferences("trth30")==1){
						download("");
					}else{
						ExclusiveManagerInterface.exclusiveOnce(
								CategoryActivityTrth.this.getParent(),
									...
									public void operationResult(OrderResult result) {
										...
										if (result != null) {
											if (result.getResCode().equals("000000")) {
												mSharedPreferencesManager.writeIntPreferences("trth30", 1);
												 System.out.println("OrderResult =" + result.getResMsg());
												 Toast.makeText(CategoryActivityTrth.this.getParent(), "支付成功，可以选择任意歌曲下载", Toast.LENGTH_LONG).show();
											}else{
												Toast.makeText(CategoryActivityTrth.this.getParent(), "支付失败", Toast.LENGTH_LONG).show();
											}
										}
	...
	private void download(String string) {
		...
		progressDialog = new ProgressDialog(CategoryActivityTrth.this
				.getParent());
	...
	public void showCheckBoxListView() throws JSONException {
		...
		if (data != null)
			data.clear();
		data = getmusics(getFromAssets("music.txt", CategoryActivityTrth.this));
