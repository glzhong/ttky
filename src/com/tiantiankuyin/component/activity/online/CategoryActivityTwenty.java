...
public class CategoryActivityTwenty extends Activity {
	 
	public static final String DOWNLOADURL = "http://120.76.235.119/selected/";
	TextView tv = null;
	ListView lv = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		rootView = LayoutInflater.from(this).inflate(R.layout.main20, null);
		setContentView(rootView);
		CommonUtils.setTitle(rootView, "10会员专区", true, false);
		...
		btn_kaitong =(Button) this.findViewById(R.id.kaitong);
		try {
			showCheckBoxListView();
		} catch (JSONException e) {
			e.printStackTrace();
		}
		btn_kaitong.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				if(mSharedPreferencesManager.readIntPreferences("twtenty")==1){
					Toast.makeText(CategoryActivityTwenty.this, "您已经是会员", Toast.LENGTH_LONG).show();
				}else{
					...
					CPManagerInterface
					.openCPMonth(
							CategoryActivityTwenty.this.getParent(),
							"698039020050006022", "1234568547",// 600967020000006114 //600967020000006022
								...
								public void operationResult(
									...
									if (null != result) {
										if (result.getResCode().equals("0")) {
											...
											mSharedPreferencesManager.writeIntPreferences("twtenty", 1);
											Toast.makeText(CategoryActivityTwenty.this.getParent(), "支付成功，可以选择任意歌曲下载", Toast.LENGTH_LONG).show();
											 //download("");
										}else{
											Toast.makeText(CategoryActivityTwenty.this.getParent(), "支付失败", Toast.LENGTH_LONG).show();
										}
									}
								}
							});
		...
		btn_download.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				for (int i = 0; i < listStr.size(); i++) {
				 ...
				 //download("");
				if(listStr.size()>10){
					Toast.makeText(CategoryActivityTwenty.this, "最多可以选中10首歌曲", Toast.LENGTH_LONG).show();
				}else if(listStr.size()==0){
					Toast.makeText(CategoryActivityTwenty.this, "请选中您喜欢的歌曲", Toast.LENGTH_LONG).show();
				}else{
					if(mSharedPreferencesManager.readIntPreferences("twtenty")==1){
						download("");
					}else{
						//System.out.println("getNowTimeStamp ==="+System.currentTimeMillis()+"");
						Toast.makeText(CategoryActivityTwenty.this, "您还未开通会员，请先开通会员！", Toast.LENGTH_LONG).show();
...
//				 download("");
	...
	private void download(String string) {
		...
		progressDialog = new ProgressDialog(CategoryActivityTwenty.this
				.getParent());
	...
	public void showCheckBoxListView() throws JSONException {
		...
		if (data != null)
			data.clear();
		data = getmusics(getFromAssets("music20.txt", CategoryActivityTwenty.this));
