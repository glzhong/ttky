 ...
 * 
 */
public abstract class EasouAsyncTask<Params, Progress, Result> {
	...
	/**
	 * 初始化缓冲队列(没有规定队列长度，是为了避免RejectedExecutionException)
	 */
	...
	private static final ThreadFactory sThreadFactory = new ThreadFactory() {
		private final AtomicInteger mCount = new AtomicInteger(1); // 一个线程安全的增减操作原子类
	 ...
	 * 线程池
	 * 
	 * CORE_POOL_SIZE 线程池维护线程的最少数量 MAXIMUM_POOL_SIZE 线程池维护线程的最大数量 KEEP_ALIVE
	 * 线程池维护线程所允许的空闲时间 TimeUnit.SECONDS 空闲时间单位 sWorkQueue
	 * 线程池所使用的缓冲队列（当线程池中的工作线程达到最大值，将会把任务放进缓冲队列） sThreadFactory 线程工厂
	 */
	...
	/**
	 * Handler的内部封装，内部处理Thread的状态
	 */
	private static final InternalHandler sHandler = new InternalHandler();

	/**
	 * 线程工作处理，是对Runnable的封装实现了Callable回调功能
	 */
	...
	public enum Status {
		...
		RUNNING,
		/**
		 * Indicates that {@link EasouAsyncTask#onPostExecute} has finished.
		 */
		FINISHED,
	}
	 ...
	 * UI thread.
	 */
	public EasouAsyncTask() {
		mWorker = new WorkerRunnable<Params, Result>() {
			public Result call() throws Exception {
				Process.setThreadPriority(Process.THREAD_PRIORITY_BACKGROUND); // 设置线程级别为后台
			...
			protected void done() {
				...
				try {
					...
					message = sHandler.obtainMessage(MESSAGE_POST_CANCEL,
							new HttpAsyncTaskResult<Result>(
									EasouAsyncTask.this, (Result[]) null));
				...
				message = sHandler.obtainMessage(MESSAGE_POST_RESULT,
						new HttpAsyncTaskResult<Result>(EasouAsyncTask.this,
								result));
				message.sendToTarget();
			}
		};
	 ...
	 *             {@link EasouAsyncTask.Status#RUNNING} or
	 *             {@link EasouAsyncTask.Status#FINISHED}.
	 */
	public final EasouAsyncTask<Params, Progress, Result> execute(
			Params... params) {
	...
	protected final void publishProgress(Progress... values) {
		sHandler.obtainMessage(MESSAGE_POST_PROGRESS,
				new HttpAsyncTaskResult<Progress>(this, values)).sendToTarget();
	}
	...
	private static class HttpAsyncTaskResult<Data> {
		@SuppressWarnings("rawtypes")
		final EasouAsyncTask mTask;
		final Data[] mData;

		@SuppressWarnings("rawtypes")
		HttpAsyncTaskResult(EasouAsyncTask task, Data... data) {
			mTask = task;
			mData = data;
		}
	}
